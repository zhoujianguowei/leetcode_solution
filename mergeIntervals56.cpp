...
vector<Interval> merge(vector<Interval>& intervals)
{
	...
	for (int i = 1; i < (signed)intervals.size(); i++)
	{
		while (i < (signed)intervals.size() && newTemp.end >= intervals[i].start)
		{
			newTemp.end = intervals[i].end>newTemp.end ? intervals[i].end : newTemp.end;
			i++;
		}
		resInterval.push_back(newTemp);
		if (i < (signed)intervals.size())
		{
			newTemp = intervals[i];
			if (i == (signed)intervals.size() - 1)
				resInterval.push_back(newTemp);
		}
